		...
		methods: {
			...
			confirmSharing() {
				...
				uni.showModal({
                    ...
                    // 点击确认共享之后，页面会自动跳转到首页
										uni.navigateBack({ // 在页面返回成功后，获取栈顶页面对象并执行其 onLoad() 方法，并将页面的 options 参数传递给 onLoad() 方法。这样可以在页面返回后刷新页面或执行其他需要的操作。
											success: () => {
												let page = getCurrentPages().pop(); //跳转页面成功之后
												console.log(page,444444444)
												if (!page) {
													return;
												} else {
													page.onLoad(page
